...
<div class="border">
    <div>
        ...
        <div class=" text-center h6 pt-2 font-weight-normal">
        <h5>Performance</h5>
      ...
      <div class="container ">
        <div class="row ">
            ...
            <div class="col-sm-6 h5 " >
                <div class="card bg-gradient-blue-white ">
                    <div class="card-body">
                        <p class="font-weight-normal"> Total Purchase : <span class="h6">Rs. 25000 </span></p>
                        <p class="font-weight-normal"> Total Saving :  <span class="h6"> Rs.  5000 </span></p>
            ...
            <div class="col-sm-6 h5 ">
                <div class="card bg-gradient-blue-red">
                    <div class="card-body  ">
                        <p class="font-weight-normal"> Your Referral Sale : <span class="h6">Rs. 45000 </span> </p>
                        <p class="font-weight-normal"> Your Referral Earning : <span class="h6">Rs. 15000 </span> </p>
...
<div class="row">
    <div class="col-md-12 ">
        <div class="card shadow-none mb-4 py-4 bg-primary pl-5">
            <div class="card-body pr-5" style="display: flex; align-items: center;">
                ...
                <div class="ml-4 pl-5">
                    <h1 class="mb-0 text-white" style="font-size: 40px;">
                        Your coupon codes
                    </h1>
                </div>
            </div>
        ...
        <div class="row mt-4">
            <div class="col-sm-3" >
                <div class="card">
                    <div class="card-body d-flex align-items-center" >
                        <p class="mb-0 mr-2">A</p>
                        <input id="inputA" type="text" class="custom-input custom-input-lg" readonly >
                        <i class="fa fa-clipboard custom-icon ml-2 text-primary" onclick="copyText('inputA')" aria-hidden="true"></i>
                    </div>
                </div>
            </div>
            <div class="col-sm-3">
                <div class="card">
                    <div class="card-body d-flex align-items-center">
                        <p class="mb-0 mr-2">B</p><br>
                        <input id="inputB" type="text" class="custom-input custom-input-lg" readonly>
                        <i class="fa fa-clipboard custom-icon ml-2 text-primary" onclick="copyText('inputA')" aria-hidden="true"></i>
                    </div>
                </div>
            </div>
            <div class="col-sm-3">
                <div class="card">
                    <div class="card-body d-flex align-items-center" >
                        <p class="mb-0 mr-2">C</p>
                        <input id="inputC" type="text" class="custom-input custom-input-lg" readonly>
                        <i class="fa fa-clipboard custom-icon ml-2 text-primary" onclick="copyText('inputA')" aria-hidden="true"></i>
                    </div>
                </div>
            </div>
            <div class="col-sm-3">
                <div class="card">
                    <div class="card-body d-flex align-items-center" >
                        <p class="mb-0 mr-2">D</p>
                        <input id="inputA=D" type="text" class="custom-input custom-input-lg" readonly>
                        <i class="fa fa-clipboard custom-icon ml-2 text-primary" onclick="copyText('inputA')" aria-hidden="true"></i>
                    </div>
                </div>
            </div>
...
@php
    ...
    if (Auth::check() && Auth::user()->shop) {
        $level = Auth::user()->shop->level;
        if ($level === 'A') {
            $discountPercentage = 0.3; // 30% discount for level A
        } elseif ($level === 'B') {
            $discountPercentage = 0.2; // 20% discount for level B
        } elseif ($level === 'C') {
            $discountPercentage = 0.1; // 10% discount for level C
        } elseif ($level === 'D') {
            $discountPercentage = 0.01; // 1% discount for level D
        }
    }
    // Ensure both base price and discount percentage are treated as numeric values
  $priceString = home_base_price($product);
preg_match('/[\d,]+\.\d+/', $priceString, $matches);
$numericPrice = str_replace([',', 'Rs'], ['', ''], $matches[0]); // Remove commas and "Rs" from the extracted string
$basePrice = (float) $numericPrice; // Convert to float

    $discountedPrice = $basePrice * (1 - $discountPercentage); // Calculate discounted price
@endphp

@if ($basePrice != $discountedPrice)
    <del class="fw-600 opacity-50 mr-1">{{ $basePrice }}</del>
@endif
<span class="fw-700 text-primary">{{ $discountedPrice }}</span>
</div>
   ...
   <div class="card">
    <div class="card-body">
        <div class="card-title text-primary">
            <!--<h6 class="mb-0">{{ translate('Top 12 Products') }}</h6>-->
             <h6 class="mb-0 text-center">{{ translate('Products') }}</h6>
             <hr>
        </div>
        <div class="row">
            @foreach ($products as $key => $product)
                <div class="col-xl-3 col-lg-4 col-md-6 col-12 col-sm-6 mb-3 ">
                    <div class="aiz-card-box border border-light rounded shadow-sm hov-shadow-md mb-2 has-transition bg-white h-100 ">
                        <div class="position-relative">
                            <a href="{{ route('product', $product->slug) }}" class="d-block">
                                <img class="img-fit lazyload mx-auto h-210px"
                                    src="{{ static_asset('assets/img/placeholder.jpg') }}"
            ...
            <div
                class="card mb-0 @if (addon_is_activated('seller_subscription')) px-4 py-5 @else p-5 h-100 @endif d-flex align-items-center justify-content-center">
                 @if (Auth::user()->shop->verification_status == 0)
                    <div class=" py-1 text-center mb-4">
                       <a href="#" class="btn btn-sm btn-primary" style="font-size: larger;color:red;">Level: 0 (Approval Pending)</a>
                    </div>
               @else
    <div class="my-2 py-1">
        <!--<a href="#" class="" style="font-size: 30px; color: blue; font-weight: bold;">Level: {{ Auth::user()->shop->level }}</a>-->
        @if(Auth::user()->shop->level == 'A')
            <img src="{{ static_asset('assets/img/a.png') }}" alt="" width="">
        @elseif(Auth::user()->shop->level == 'B')
           <img src="{{ static_asset('assets/img/b.png') }}" alt="" width="">
        @elseif(Auth::user()->shop->level == 'C')
            <img src="{{ static_asset('assets/img/c.png') }}" alt="" width="">
        @elseif(Auth::user()->shop->level == 'D')
            <img src="{{ static_asset('assets/img/d.png') }}" alt="" width="">
        @endif
    </div>
@endif
